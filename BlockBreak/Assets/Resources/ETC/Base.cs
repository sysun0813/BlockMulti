...
public class Base : MonoBehaviour
{
    ...
    public GameObject P_Ball, P_GreenOrb, P_Block;
    ...
    void Update_GM()
    {
        ...
        shotable = true;
        for (int i = 0; i < BallGroup.childCount; i++)
            if (BallGroup.GetChild(i).GetComponent<Base>().isMoving) shotable = false;
    ...
    void FixedUpdate_GM()
    {
        ...
        {
            timerCount = 0;
            BallGroup.GetChild(launchIndex++).GetComponent<Base>().Launch(gap);
    ...
    void Start_BALL() => GM = GameObject.FindWithTag("GameManager").GetComponent<Base>();
